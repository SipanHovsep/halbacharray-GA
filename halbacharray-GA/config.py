...
InnerBoreDiameter = 300 * 1e-3  # Inner Diameter of the Ring (m)
OuterBoreDiameter = 500 * 1e-3  # Outer Diameter of the Ring (m)
...
bandRadiiGap = np.linspace(0.01, 0.08, 70)     # Space between Bands (mm)
magnetSpace = np.linspace(0.01, 0.1, 70)     # Space Between Magnets (mm)
bandSep = np.linspace(0.01, 0.02, 70)      # Space between Bore and 1st band (mm)
...
T_target = 0.15                # Target field strength in Tesla
...
DSV = 0.7 * InnerBoreDiameter       # Diameter of the Spherical Volume, as a % of the inner bore diameter (mm)
...
CXPB, MUTPB = 0.6, 0.5              # Crossover probability and mutation probability
